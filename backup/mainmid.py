...
def train_and_evaluate_model(model, X, y, X_test, model_name: str, n_splits=5):
    ...
    for fold, (train_idx, val_idx) in enumerate(tscv.split(X)):
        ...
        y_pred_val = step_model.predict(X_val_fold)
        
        #pass X_val_fold to access lag_1
        metrics = evaluate_predictions(y_val_fold, y_pred_val, X_val_fold)
        
        for k in fold_metrics:
            if k in metrics:
                fold_metrics[k].append(metrics[k])
                
        #print concise fold info
